...
```{r transformations or modifications you made to the dataset}
...
## IExecute a summary() function on two variables defined in the previous step to compare the model results. What are the R2 and Adjusted R2 statistics? Explain what these results tell you about the overall model. Did the inclusion of the additional predictors help explain any large variations found in Sale Price?
...
```{r Calculate the standardized residuals }
...
* Twice/Thrice of 0.000310  = 0.00093 (0.000310*3) - Leverage finding - Most of the cases are 
within the boundaries ( < 0.00093), three times of the average.
...
* Conservative rule suggest values below 1 and above 3, will raise alarm. In our case D-W stats 
  reported 0.52 which is less than 1 raise the alarm and p-value = 0 is again a concern of no co-relationship.
...
* multicollinearity - VIF values are all below 10, and tolerance stats all well above 0.2 and 
  Average VIF value i.e 2 is above 1 is a concern to conclude no collinearity within the data
...
ggplot(Model_02, aes(Model_02$fitted.values, Model_02$residuals)) + geom_point() + geom_smooth(method = "lm", colours = "Blue") + labs(x = "Fitted Values", y = "Stu Residuals")
